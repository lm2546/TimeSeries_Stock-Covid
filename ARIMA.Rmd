...
```{r}
fit2 <- Arima(johnson.ts,order=c(1,1,3),include.drift=TRUE)
autoplot(johnson.ts,series='Data') +
  autolayer(meanf(johnson.ts,150),PI=FALSE,series='Mean') +
  autolayer(naive(johnson.ts,150),PI=FALSE,series='Naive') +
  autolayer(rwf(johnson.ts,150),PI=FALSE,series='Drift') +
...
```{r message=FALSE, warning=FALSE}
library(Metrics)
a2 <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(a2) <- c('Method','MAE','MSE')
a2[1,] <- c('Mean',mae(as.numeric(f1$mean),pfizer.test),
           mse(as.numeric(f1$mean),pfizer.test))
a2[2,] <- c('Naive',mae(as.numeric(f2$mean),pfizer.test),
           mse(as.numeric(f2$mean),pfizer.test))
a2[3,] <- c('Drift',mae(as.numeric(f3$mean),pfizer.test),
           mse(as.numeric(f3$mean),pfizer.test))
a2[4,] <- c('ARIMA',mae(as.numeric(pred1$mean),pfizer.test),
...
```{r}
fit2 <- Arima(pfizer.ts,order=c(1,1,3),include.drift=TRUE)
autoplot(pfizer.ts,series='Data') +
  autolayer(meanf(pfizer.ts,150),PI=FALSE,series='Mean') +
  autolayer(naive(pfizer.ts,150),PI=FALSE,series='Naive') +
  autolayer(rwf(pfizer.ts,150),PI=FALSE,series='Drift') +
  autolayer(forecast(fit2,150),PI=FALSE, series='AR') +
  xlab("Date") + ylab("Value") +
...
```{r message=FALSE, warning=FALSE}
library(Metrics)
a3 <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(a3) <- c('Method','MAE','MSE')
a3[1,] <- c('Mean',mae(as.numeric(f1$mean),moderna.test),
           mse(as.numeric(f1$mean),moderna.test))
a3[2,] <- c('Naive',mae(as.numeric(f2$mean),moderna.test),
           mse(as.numeric(f2$mean),moderna.test))
a3[3,] <- c('Drift',mae(as.numeric(f3$mean),moderna.test),
           mse(as.numeric(f3$mean),moderna.test))
a3[4,] <- c('ARIMA',mae(as.numeric(pred1$mean),moderna.test),
...
```{r}
fit2 <- Arima(moderna.ts,order=c(1,1,3),include.drift=TRUE)
autoplot(moderna.ts,series='Data') +
  autolayer(meanf(moderna.ts,150),PI=FALSE,series='Mean') +
  autolayer(naive(moderna.ts,150),PI=FALSE,series='Naive') +
  autolayer(rwf(moderna.ts,150),PI=FALSE,series='Drift') +
  autolayer(forecast(fit2,150),PI=FALSE, series='AR') +
  xlab("Date") + ylab("Value") +
